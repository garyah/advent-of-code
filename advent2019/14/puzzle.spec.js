describe("2019 day 14", function() {
  ...
  const solve_p2 = (recipes = [[[[]]]]) => {
    solve(recipes);
    let totalFuel = 0;
    const fuelRecipe = recipeMap.get('FUEL');
    while (totalOre < 1000 * 1000 * 1000 * 1000) {
      walkItemList(fuelRecipe.itemList);
      totalFuel++;
    }
    return totalFuel;
  ...
  it("can solve puzzle part 2", () => {
    ...
    // const actual = data.map((data) => solve_p2(parse(data)));
    const expected = [
        // ???,
        // ???,
        // 82892753,
